...
// grid variables
const uint8_t hOffset = (TFT_WIDTH - GRID_WIDTH)/2;
const uint8_t vOffset = (TFT_HEIGHT - GRID_HEIGHT) / 2;
const uint8_t dHeight = GRID_HEIGHT/8;
...
// ------------------------
    ...
    tft.fillTriangle(0, cPos - 5, hOffset, cPos, 0, cPos + 5, color);//tft.fillTriangle(x0, y0, x1, y1, x2, y2, color)
	if(highlight)
...
// ------------------------
  ...
  //tft.setCursor(301, 4);
	...
	// -----------------
	tft.setTextColor(ILI9341_GREEN, ILI9341_BLACK);
	tft.setCursor(105, GRID_HEIGHT + vOffset + 4);
	if(currentFocus == L_timebase)
		tft.drawRect(100, GRID_HEIGHT + vOffset, 70, vOffset, ILI9341_WHITE);
  ...
  tft.print("/div");
	...
	// -----------------
	tft.setTextColor(ILI9341_PINK, ILI9341_BLACK);
	tft.setCursor(200, GRID_HEIGHT + vOffset + 4);//230
	if(currentFocus == L_triggerType)
		tft.drawRect(195, GRID_HEIGHT + vOffset, 35, vOffset, ILI9341_WHITE);//225
	...
	if(currentFocus == L_triggerEdge)
		tft.drawRect(236, GRID_HEIGHT + vOffset, 15, vOffset + 4, ILI9341_WHITE);//266
...
// ------------------------
	...
	if(clearStats){
    if(wStats.freq < 100.00){
		  tft.fillRect(60, 20, 60, 10, ILI9341_BLACK);//freq
    }
    else{
      tft.fillRect(60, 20, 80, 10, ILI9341_BLACK);//freq
    }
    if(wStats.cycle < 10.00) {
      tft.fillRect(60, 30, 42, 30, ILI9341_BLACK);//cycle,pw,duty
    }
    else{
      tft.fillRect(60, 30, 60, 30, ILI9341_BLACK);//cycle,pw,duty
    }
	}
	
	if(wStats.pulseValid)	{
		tft.setCursor(60, 20);
    if(wStats.freq >= 1000.0){
      tft.print(wStats.freq/1000.0,3); tft.print(" kHz");
    }
    else{
      tft.print(wStats.freq,3); tft.print(" Hz");
    }
		tft.setCursor(60, 30);
...
void banner()	{
	...
	tft.print("Firmware by Ardyesp, web:");
	tft.setTextColor(ILI9341_YELLOW, ILI9341_BLACK);
	...
	tft.print("Modified by barty12, web:");
	tft.setTextColor(ILI9341_GREEN, ILI9341_BLACK);
	tft.setCursor(40, 140);
	tft.print("http://barty12.wz.cz");
