...
// ------------------------
	...
	if(lDelay < 0)	{
        ...
        while(keepSampling){
            int32_t* adc1base = (int32_t*)0x40012400;
            int32_t* adc2base = (int32_t*)0x40012800;
            uint16_t* adc1data = (uint16_t*)0x4001244C;
            uint16_t* adc2data = (uint16_t*)0x4001284C;

			//wait until end of conversion
			//while(!(ADC1->regs->SR & ADC_SR_EOC_BIT));
            //while(!(ADC2->regs->SR & ADC_SR_EOC_BIT));
            //while(!(*adc1base & 0b0010));
            //while(!(*adc2base & 0b0010));
            while(((*adc1base) << 30) >= 0);
            while(((*adc2base) << 30) >= 0);
            ch1Capture[sIndex] = *adc1data;
            ch2Capture[sIndex] = *adc2data;
            ++sIndex;
            if(sIndex == NUM_SAMPLES){
                sIndex = 0;
                snapMicros();
